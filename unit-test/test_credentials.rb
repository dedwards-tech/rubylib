...
class UnitTest_Credentials < Test::Unit::TestCase
  class SshCredentialTest
    include SshCredential
    def initialize
      # create instancee of credential (@items)
  ...
  def test_ssh_credential_init
    puts("test case #{__method__.to_s} - exec")

    ssh_cred = SshCredentialTest.new()
  ...
  def test_ssh_credential_from_json
    puts("test case #{__method__.to_s} - exec")

    json_str = '{ "user_name": "joeblow", "user_pwd": "7654321", "user_group": "double", "server_ip": "192.168.1.101"}'
    ssh_cred = SshCredentialTest.new()
    ssh_cred.from_json(json_str)
    assert_equal(ssh_cred.server_ip, '192.168.1.101', "Invalid server_ip value.")
    assert_equal(ssh_cred.user_name, 'joeblow', "Invalid user_name value.")
    assert_equal(ssh_cred.user_pwd, '7654321', "Invalid user_pwd value.")
    assert_equal(ssh_cred.user_group, 'double', "Invalid default user_group value.")
  end

  def test_ssh_credential_from_hash
    puts("test case #{__method__.to_s} - exec")

    test_h = { :user_name => 'johnny', :user_pwd => '8675309', :server_ip => '192.168.1.102', :ignore_me => 'testing123' }
    ssh_cred = SshCredentialTest.new()
    ssh_cred.from_h(test_h)
    assert_equal(ssh_cred.server_ip, '192.168.1.102', "Invalid server_ip value.")
    assert_equal(ssh_cred.user_name, 'johnny', "Invalid user_name value.")
    assert_equal(ssh_cred.user_pwd, '8675309', "Invalid user_pwd value.")
  ...
  def test_ssh_credential_to_json
    puts("test case #{__method__.to_s} - exec")
